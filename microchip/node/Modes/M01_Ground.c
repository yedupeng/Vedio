...
typedef struct
{
	...
	uint16_t pre_enter_mode_counter1;
}MODE_INF;
...
static void M01_Ground_MainFunc()
{
	...
	bool sticks_in_neutral =
		in_symmetry_range_offset_float(throttle , 5 , 50) && \
	...
	/*判断进入模式*/
		//通道5拉高触发模式转换
		if(rc->data[5]<60&&rc->data[5]>40&&sticks_in_neutral)
		{
			if(++Mode_Inf->pre_enter_mode_counter1>=100)
			{
				change_Mode(35);
			}
		}
		else
			Mode_Inf->pre_enter_mode_counter1=0;
}