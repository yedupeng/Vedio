...
class Function():
    ...
    def recogniced_line(self,img):
        ...
        rows = Img.shape[0]
        cols = Img.shape[1]
        part1 = Img[0:rows//3,0:cols]
        part2 = Img[rows//3:rows*2//3,0:cols]
        part3 = Img[rows*2//3:rows,0:cols]
        erode = cv2.erode(part1,None,1)
        blurred = cv2.GaussianBlur(part1, (7, 7), 0)
        edges = cv2.Canny(blurred,170,255)
        ...
        try:
            are_max = max(area_list, key=cv2.contourArea)
            rect = cv2.minAreaRect(are_max)
            box = cv2.boxPoints(rect)
            cv2.drawContours(part1, [np.int0(box)], -1, (0, 255, 255), 2)
            left_point_x = np.min(box[:, 0])
            right_point_x = np.max(box[:, 0])
            center = (left_point_x+right_point_x)/2
        except:
            pass
        hstack = np.hstack([part1,part2,part3])
        cv2.imshow("Img1",edges)

        cv2.imshow("gray",hstack)
