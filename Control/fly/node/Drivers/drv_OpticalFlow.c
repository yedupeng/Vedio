	...
	while( ( UART1->FR & (1<<4) ) == false	)
	{
		...
		{	//接收包尾
			if( rdata == 0x55 )
			{
				vector3_float vel;
				float ultra_height;
				dbg_height = ultra_height;
				bool ultra_height_available = get_Estimated_Sensor_Position_z( &ultra_height , default_tfmini_sensor_index );
          ...
          if( ultra_deadband < 5 )
					...
					PositionSensorSetInavailable( default_tfmini_sensor_index );
			}
			rc_counter = 0;
		}
	}
